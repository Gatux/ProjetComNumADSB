...
\begin{figure}[h]
	\centering
		\includegraphics[scale=0.4]{img/rsl.png}
			\caption{Fonction d'autocorrélation $R_{sl}(\tau)$}
			\label{rsl}
\end{figure}
...
Le bloc de réception se voit ajouter le calcul de la fonction REF permettant d'estimer les deux paramètres $\delta_t$ et $\delta_f$. Il devient alors possible de démoduler le signal, puis de calculer et de tracer la nouvelle courbe du TEB en fonction du SNR.
